...
function App() {
  const { lists, usedAdvertisemens } = useSelector((state: RootState) => ({
    lists: Object.entries(state.lists).map(([id, list]) => ({ id, ...list })),
    ...state.ad
  }));
    ...
    <DndProvider backend={HTML5Backend}>
      <DndWrapper>
        <div className="cards-container">
          {!!lists.length &&
            lists.map((list, index) => <Column key={list.id} list={list} index={index} />)}
          {isAddingColumn ? (
