...
export const DndWrapper: React.FC<DndWrapperProps> = ({ children }) => {
  ...
  const { draggableCard, hoveredCard, draggableList, hoveredList } = useSelector(
    (state: RootState) => state.dragging
  );
  ...
  return (
    <div ref={drop} className={`${styles.dndWrapper} ${styles[theme]} p-xl`}>
      {children}
    </div>
  );
};
